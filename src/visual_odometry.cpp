...
void VisualOdometry::featureMatching()
{
    ...
    {
        if (m.distance < max<double>(min_dis * match_ratio_, 30.0))
        {
...
void VisualOdometry::poseEstimationPnP()
{
    ...
    cv::Mat K = (cv::Mat_<double>(3, 3) << ref_->camera_->fx_, 0, ref_->camera_->cx_,
                 ...
                 0, 0, 1);
    cv::Mat rvec, tvec, inliers,RR;
    cv::solvePnPRansac ( pts3d, pts2d, K, cv::Mat(), rvec, tvec, false, 100, 4.0, 0.99, inliers,cv::SOLVEPNP_ITERATIVE );
    ...
    pose->setEstimate ( g2o::SE3Quat (
        Rvec, Vector3d ( tvec.at<double> ( 0,0 ), tvec.at<double> ( 1,0 ), tvec.at<double> ( 2,0 )
    )));
